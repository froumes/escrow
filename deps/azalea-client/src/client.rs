...
async fn run_schedule_loop(
    ...
    //
    // Patched by frikadellen-baf: run the Update schedule at 300 fps instead of
    // the vanilla 60 fps. Because packets are only handled during Updates, this
    // reduces average packet-detection latency from ~8.3 ms to ~1.65 ms, helping
    // purchases complete in fewer server ticks on borderline connections.
    const UPDATE_DURATION_TARGET: Duration = Duration::from_micros(1_000_000 / 300);
