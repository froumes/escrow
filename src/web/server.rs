...
// ── Setup-mode web server (first-time authentication) ────────

/// Lightweight shared state used during the first-time setup flow.
/// Only carries what the panel + chat WebSocket need — no bot/ws_client.
#[derive(Clone)]
pub struct SetupWebState {
    pub chat_tx: broadcast::Sender<String>,
    pub web_gui_password: Option<String>,
    pub valid_sessions: Arc<Mutex<HashSet<String>>>,
}

/// Start a minimal web server for the first-time setup flow.
///
/// Only serves the panel HTML, the login endpoint, and the chat WebSocket
/// so that users can see the Microsoft & Coflnet login links in the web UI.
pub async fn start_setup_web_server(state: SetupWebState, port: u16) {
    let has_password = state
        .web_gui_password
        .as_ref()
        .map(|p| !p.is_empty())
    ...
    let app = Router::new()
        .route("/", get(index_page))
        .route("/api/login", axum::routing::post(setup_login))
        .route("/api/status", get(setup_status))
        .route("/api/chat/ws", get(setup_chat_ws_handler))
        .layer(axum::middleware::from_fn(move |req: Request, next: Next| {
            let s = auth_state.clone();
            async move { check_setup_auth(s, req, next).await }
        }))
        .with_state(state);
    ...
    if has_password {
        info!(
            "[Setup] Web panel starting on http://{} (password protected)",
            addr
        );
    } else {
        info!(
            "[Setup] Web panel starting on http://{} (no password)",
            addr
        );
    }
    ...
    let listener = match tokio::net::TcpListener::bind(&addr).await {
        Ok(l) => l,
        Err(e) => {
            error!("[Setup] Failed to bind web server on {}: {}", addr, e);
            return;
        }
    };

    if let Err(e) = axum::serve(listener, app).await {
        error!("[Setup] Web server error: {}", e);
    }
}

/// Authentication middleware for setup mode (same logic, different state type).
async fn check_setup_auth(
    s: SetupWebState,
    req: Request,
    next: Next,
) -> Response {
...
async fn setup_login(
    State(s): State<SetupWebState>,
    Json(payload): Json<LoginPayload>,
) -> impl IntoResponse {
    let expected = match &s.web_gui_password {
        Some(p) if !p.is_empty() => p,
...
/// Minimal status endpoint for setup mode — the panel polls this.
...
async fn setup_chat_ws_handler(
    ws: WebSocketUpgrade,
    State(s): State<SetupWebState>,
) -> impl IntoResponse {
    ws.on_upgrade(move |socket| handle_setup_chat_ws(socket, s))
}

async fn handle_setup_chat_ws(mut socket: WebSocket, state: SetupWebState) {
    let mut rx = state.chat_tx.subscribe();
    ...
    debug!("[Setup] WebSocket client disconnected");
}
